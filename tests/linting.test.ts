...
tap.test("Endless Sky and the filesystem", async (t) => {
  let pluginDir = "";
  ...
  t.before(() => {
    ...
    fs.writeFileSync(
      path.join(coreDir, "SConstruct"),
      "scons is to cmake as mercurial is to git\n"
    );
  });

  t.teardown(() => {
    fs.rmdirSync(pluginDir, { recursive: true });
    fs.rmdirSync(coreDir, { recursive: true });
  });

  t.test(
    "withPreparedFilesytem should have config dir when the function runs",
    async () => {
  ...
  t.test("should have the plugin folder symlinked in", async (t) => {
  ...
  t.test("should be able to read stderr warnings for a plugin", async (t) => {
    const setUpFile = path.join(pluginDir, "data", filename);
    assert.strictEqual(fs.existsSync(setUpFile), true);
  ...
  t.test("should be able to read stderr warnings for core files", async (t) => {
...
tap.test("parse", (t) => {
  t.test("find data node errors", (t) => {
    const matches = [...sampleOutput.toString().matchAll(dataNodeError)];
    assert.strictEqual(matches.length, 6);
    assert.strictEqual(
      matches[0].groups!.file!,
      "/Users/tomb/endless-sky/data/coalition/coalition ships.txt"
    );
    assert.deepStrictEqual(matches[0].groups!.lineno!, "213");
    assert.deepStrictEqual(
      matches[0].groups!.msg!,
      "Skipping unrecognized root object:"
    );
    t.end();
  });

  t.test("find ship entity errors", (t) => {
    const matches = [...sampleOutput.toString().matchAll(shipEntityError)];
    assert.strictEqual(matches.length, 6);
    t.end();
  });

  t.test("find ship missing outfit errors", (t) => {
    const matches = [
      ...sampleOutput.toString().matchAll(shipMissingEquippedOutfit),
    ];
    assert.strictEqual(matches.length, 4);
    t.end();
  });

  t.test("finds all errors", (t) => {
