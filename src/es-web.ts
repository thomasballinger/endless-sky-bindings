...
export default async function libFactory(): Promise<ESLib> {
  ...
  const mod = await import("./lib-web.mjs");
  const libFactory = mod.default;
  const url = new URL(`./lib-web.data`, import.meta.url);
  const ab = await (await fetch('' + url)).arrayBuffer();
  const Module = {
    getPreloadedPackage: function (remotePackageName: string, remotePackageSize: string) {
      return ab;
    },
  };
...
export async function loadedEsLib(loadArgs: string[]): Promise<ESLib> {
  ...
  await esLib.GameDataBeginLoad(["--resources", "/", "--config", "/", ...loadArgs]); // this takes a couple seconds`;
  return esLib;
}
